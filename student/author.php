<?php 
    ...
    $sql='SELECT * FROM material WHERE  author_name = "'.$filtervalues.'" LIMIT ' . $this_page_first_result . ',' .  $results_per_page ;
    ...
    <div class="container" style="margin:20px auto;">
        ...
        <nav aria-label="Page navigation example" style="float:right;">
            <ul class="pagination pagination-circle">
                <li class="page-item">
                    <a class="page-link" href="author.php?page=1">First</a>
                </li>
                <li class="page-item">
                    <a class="page-link" href="author.php?page=<?php if($page==1){echo $page;}else{echo ($page-1);} ?>" aria-label="Previous">
                    ...
                    if($last_five>=$number_of_pages)
                    {
                        ...
                        for ($page_num=$first_num;$page_num<=$number_of_pages;$page_num++)
                        {
                            if($page_num!=0)
                            {
                                if($page_num == $page)
                                {
                                    echo '<li class="page-item active" aria-current="page"> <a class="page-link" href="author.php?page='
                                    .$page_num.'">'.$page_num.'<span class="visually-hidden">(current)</span></a></li>';
                                }
                                else{
                                    echo '<li class="page-item"><a class="page-link" href="author.php?page='.$page_num.'">' . $page_num . '</a></li>';
                                }
                            }
                        }
                    }
                    else
                    {
                        for ($page_num=$page;$page_num<=($page+4);$page_num++) {
                            if($page_num<=$number_of_pages)
                            {
                                if($page_num == $page)
                                {
                                    echo '<li class="page-item active" aria-current="page"> <a class="page-link" href="author.php?page='.$page_num.'">'.$page_num.'<span class="visually-hidden">(current)</span></a></li>';
                                }
                                else{
                                    echo '<li class="page-item"><a class="page-link" href="author.php?page='.$page_num.'">' . $page_num . '</a></li>';
                                }
                ...
                ?>
                <li class="page-item">
                    <a class="page-link" href="author.php?page=<?php if($page==$number_of_pages){echo $page;}else{echo ($page+1);} ?>" aria-label="Next">
                        <span aria-hidden="true">&raquo;</span>
                    </a>
                </li>
                <li class="page-item">
                    <a class="page-link" href="author.php?page=<?php echo $number_of_pages; ?>">Last</a>
                </li>
            </ul>
        </nav>
        <?php
